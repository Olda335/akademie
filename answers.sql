...
SELECT vodp.year_payroll AS aktual_year , vodp.payroll AS aktual, vodp2.payroll AS prev ,vodp2.year_payroll AS year_prev, vodp.branch  ,
round((vodp.payroll - vodp2.payroll)/vodp2.payroll*100,2) AS growth
...
SELECT DISTINCT down.branch
FROM 
(SELECT vodp.branch AS branch, 
round((vodp.payroll - vodp2.payroll)/vodp2.payroll*100,2) AS growth
FROM v_oldrich_vesely_project_sql_primary_final  vodp 
JOIN v_oldrich_vesely_project_sql_primary_final  vodp2 
	ON vodp.year_payroll = vodp2.year_payroll + 1
...
LEFT JOIN v_growth_payroll vgp
	ON vgp2.branch = vgp.branch
 	AND vgp2.growth_percent <0 
GROUP BY vgp2.branch;
